...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $idSrValue = intval($_POST['idSrValue']);
    $drawingBomInputValue = $_POST['drawingBomInputValue'];
    $prod_line = $_POST['selectedCheckboxes'];

    
    $sql = "UPDATE sr 
                  SET design_drawing = '$drawingBomInputValue', production_line = '" . implode(",", $prod_line) . "'
                  WHERE id = $idSrValue";

    $stmt = $db2->prepare($sql);

    if ($stmt) {
        $stmt->bind_param("sssi", $drawingBomInputValue, implode(",", $prod_line), $idSrValue);
        ...
        echo "Error in preparing statement: " . $db2->error;
    }

    $sent_leader = $idSrValue;
    if($sent_leader==""){$sent_leader = 0;}
    if($sent_leader != 0){
        $date_checked = date('Y-m-d');
        $Query = "update sr set id_sr_status='5',pass_en_rd_by='$sessionName', pass_en_rd_date='$date_checked' where id = '$sent_leader' ";
