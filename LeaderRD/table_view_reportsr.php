...
if($del==''){$del=-1;}
if($del != -1){
	$Querydel = "select rd_report_file from sr where id = '$id_sr'"; 
	$Resultdel = $db2->query($Querydel);
	$linedel=mysqli_fetch_array($Resultdel);
	$all_file = $linedel['rd_report_file'];
	...
	$Queryup = "update sr set rd_report_file = '$new_update' where id = '$id_sr'"; 
	...
	if($separate_file[0] != ""){
		for($i=0;$i<count($separate_file);$i++){?>
			<tr>
				...
				<td class="name" style="cursor:pointer" onclick="window.open('uploadfile/sr_report_folder/<?echo $separate_file[$i];?>',null,'height=500,width=500,status=yes,toolbar=no,menubar=no,location=no,resizable=yes')"><?echo $separate_file[$i];?></td>
